...
from ectoolkits.structures.tools import smiles2xyz,get_packmol_inp,get_exclude_water_count
...
class Slab(Atoms):
    ...
    def generate_components_symmetry_box(self, 
                                            water_box_len, 
                                            ads_components_smiles: list,
        ...
        os.mkdir('gen_water')
        logger.info("Generate Packmol Input: gen_wat_box.inp")
        inp_path = os.path.join("gen_water", "gen_wat_box.inp")
        with open(inp_path, 'w') as f:
            lines = [
                "# Packmol input generated by Python",
                "tolerance 2.0",
                "filetype xyz",
                "output watbox.xyz\n"
            ]
