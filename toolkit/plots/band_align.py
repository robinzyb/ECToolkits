...
def plot_multiple_ba(ax, ba_dict):
    ...
    for idx, (name, be_data) in enumerate(ba_dict.items()):
        plot_one_ba_sc(ax, idx, vbm=be_data["vbm"], cbm=be_data["cbm"], width=width)


def plot_one_ba_diff(ax, idx, vbm, cbm, ref_vbm, ref_cbm):
    ax.annotate('', xy=(idx, ref_vbm), xytext=(idx, vbm), arrowprops=dict(arrowstyle='<->', color='C0'))
    ax.annotate('', xy=(idx, ref_cbm), xytext=(idx, cbm), arrowprops=dict(arrowstyle='<->', color='C3'))
...
def plot_multiple_ba_diff(ax, ba_dict, key_list, ref_key):
    ...
    for idx, (name, be_data) in enumerate(ba_dict.items()):
        if name in key_list:
            plot_one_ba_diff(ax, idx, be_data["vbm"], be_data["cbm"], ref_vbm=ref_vbm, ref_cbm=ref_cbm)
...
def plot_band_alignment(ba_dict, show_diff=False, vac_value=False):
    ...
    if show_diff:
        plot_multiple_ba_diff(ax, ba_dict, key_list=name_list[1:], ref_key=first_key)
